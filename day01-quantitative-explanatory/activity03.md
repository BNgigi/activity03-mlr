...
hfi_2016 <- dplyr::filter(hfi, year == 2016)
```
...
``` r
# Review the about page of the data set to select additional numeric variables
# Let's choose `pf_identity` as an additional numeric variable to describe its distribution

# Load necessary libraries
library(dplyr)

# Describe distributions of selected numeric variables
...
``` r
summary(hfi_2016$pf_identity)
```

    ##    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
    ##   0.000   5.000   8.000   6.976   9.250  10.000

``` r
# Optionally, you can visualize the distributions using histograms
hist(hfi_2016$pf_score, main = "Distribution of Personal Freedom Score (2016)", xlab = "Personal Freedom Score")
```

![](activity03_files/figure-gfm/unnamed-chunk-3-1.png)<!-- -->

``` r
hist(hfi_2016$pf_expression_control, main = "Distribution of Personal Freedom: Expression and Information Control (2016)", xlab = "Personal Freedom: Expression and Information Control")
```

![](activity03_files/figure-gfm/unnamed-chunk-3-2.png)<!-- -->

``` r
hist(hfi_2016$pf_identity, main = "Distribution of Personal Freedom: Identity and Relationships (2016)", xlab = "Personal Freedom: Identity and Relationships")
```

![](activity03_files/figure-gfm/unnamed-chunk-3-3.png)<!-- -->
...
``` r
hfi_2016 %>% 
  select(pf_score, pf_expression_control, pf_identity) %>% 
  ggpairs()
```
...
``` r
mlr_mod <- lm_spec %>% 
fit(pf_score ~ pf_expression_control + pf_identity, data = hfi_2016)
...
```
    ...
    ##   <chr>                    <dbl>     <dbl>     <dbl>    <dbl>
    ## 1 (Intercept)              3.49     0.118       29.4 3.00e-66
    ## 2 pf_expression_control    0.399    0.0215      18.6 1.04e-41
    ## 3 pf_identity              0.216    0.0161      13.4 6.67e-28
